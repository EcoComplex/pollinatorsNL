...
#plant parameters
plant.params<-read.csv(paste0(wd.path,"/plant_parameters_ex1.csv")) #ex
...
###Histogram of foraging distances

# foraging distance of one pollinator
#
poll.names<-c("sol_nest", "eusocial", "eusocial", "euscoial", "sol_nest", "sol_nest", "sol_no_nest", "sol_no_nest")
par(mfrow=c(4,2))
for (i in 1:8){
res <- mdl %>% filter(pollinator_species==1) %>%  select(run, plant_patch,foraging_distance) 
hist(res$foraging_distance, breaks=100, main=poll.names[i], xlab="foraging distance per visit")
}
...
soc.col<-brewer.pal(n = 4, name = "Set2")
...
for (i in 1:length(s)){
  ...
  boxplot(t$number_visits ~ t$pollinator_species, data = no.visits, cex=1, main=paste("Ex1",s[i]), xlab="pollinator species", ylab="total no of plant visits", col=soc.col[box.col.sp]) 
}
plot.new()
legend("center", legend=c("sol.no_nest", "sol.nest", "prim_eusocial", "high_eusocial"), col=soc.col, lwd=3, lty=1, cex=2,  bty = "n")
#dev.off()

#### to get total number of visits per habitat per seed percent####
####
no.visits<-mdl %>% group_by(run, seed_percent, habitat) %>% summarize(number_visits = n() ) 
#png(paste0(dirF, "Boxplot_Poll.sp_tot_vis_per_seed_ex6",".png"),width=7*ppi, height = 4*ppi, res=ppi)
#png(paste0(dirF, "Boxplot_Poll.sp_tot_vis_per_seed",".png"), width=4.5*ppi, height = 4*ppi, res=ppi)
#Color habitats
plant.col<-brewer.pal(n = 4, name = "Set3")
plant.habs<-rep(1:4, 4)

#no.of visits per plant species for each seed percent per habitat
par(mfrow=c(2,3))
par(mar=c(4,4,3,1), oma=c(2,2,2,2))
s<-c(1e-05, 1e-04, 1e-03, 1e-02, 1e+00) #seed.percent used in the loop
for (i in 1:length(s)){
  t<-filter(no.visits, seed_percent==s[i])
  boxplot(t$number_visits ~ t$habitat, data = no.visits, cex=1, main=paste("Ex1",s[i]), xlab="plant species", ylab="total no of plant visits", col=plant.col[plant.habs]) 
}
plot.new()
legend("center", legend=c("hab1", "hab2", "hab3", "hab4"), col=plant.col, lwd=3, lty=1, cex=2,  bty = "n")
#dev.off()

#### to get total number of visits per plant sp per seed percent####
# Total Number of visits 
...
#no.of visits per plant species for each seed percent
par(mfrow=c(2,3))
par(mar=c(4,4,3,1), oma=c(2,2,2,2))
s<-c(1e-05, 1e-04, 1e-03, 1e-02, 1e+00) #seed.percent used in the loop
for (i in 1:length(s)){
  t<-filter(no.visits, seed_percent==s[i])
  boxplot(t$number_visits ~ t$plant_species, data = no.visits, cex=1, main=paste("Ex1",s[i]), xlab="plant species", ylab="total no of plant visits", col=plant.col[plant.habs]) 
}
plot.new()
legend("center", legend=c("hab1", "hab2", "hab3", "hab4"), col=plant.col, lwd=3, lty=1, cex=2,  bty = "n")
#dev.off()
...
par(mfrow=c(1,1))
plot(total.vis, seed, type="n", xlab="seed percent", ylab="total plant visits by poll. ind.", ylim=c(100, 700), xlim=c(1,5), labels=F, main="ex1")
...
plant.vis.data<-list()
poll.data<-list()
plant.no<-length(plant.params$plant_species)
tot.vis<-matrix(0, length(file.list), plant.no)
seed<-c()
all.dat<-c()
for (z in 1:length(file.list)){ 
  ...
  plants.convis<-matrix(0, poll.no, plant.no)
  sp.no<-c()
  ags.id<-c()
  s<-c()
  r<-c()
...
all.dat
all.dat.one.file <- bind_rows(all.dat)
#all.dat.one.file<-as_tibble(all.dat.one.file)
#all.dat.one.file %>% gather(plant_sp, convis, all.dat.one.file[,c(1:2)]) 

tot.plant.vis.seed<-as.data.frame(cbind(tot.vis, seed))
plant.col<-brewer.pal(n = 4, name = "Set3")
plant.habs<-rep(1:4, 4)
col.id.s<-which(colnames(tot.plant.vis.seed)=="seed")
#png(paste0(dirF, "Boxplot_plants_con.vis_per_seed",".png"),width=7*ppi, height = 4*ppi, res=ppi)
par(mfrow=c(2,3)) 
s<-unique(all.dat.one.file$s)
for (i in 1:length(s)){
  t<-tot.plant.vis.seed[which(tot.plant.vis.seed$s==s[i]),]
  boxplot(t[-col.id.s], data = tot.plant.vis.seed, main=paste("Ex6",s[i]), xlab="plant sp", ylab="tot.con.visits",col=plant.col[plant.habs]) 
}
#plot.new()
#barplot(apply(tot.plant.vis.seed[,-col.id.s], 2, mean),  col=plant.col[plant.habs],  xlab="plant sp", ylab="mean Tot.sum.con.visits")
#legend("center", legend=c("sol.no_nest", "sol.nest", "eusocial"), col=soc.col, lwd=3, lty=1, cex=2,  bty = "n")
#dev.off()


#Total number of consecutive visits to a plant per seed percent
#boxplot etc
par(mfrow=c(1,1)) 
con.vis.sum<-cbind(tot.plant.vis.seed[, col.id.s], apply(tot.plant.vis.seed[, -col.id.s], 1, sum))
