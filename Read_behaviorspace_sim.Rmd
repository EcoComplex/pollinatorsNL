...
#
mdl <- lapply( simul_fn, function(x){ read_delim(paste0("Simulations/",x),";") %>% mutate(filename=x) })
...
mdl %>% group_by(filename,pollinator_species) %>% summarize(foraging_distance = max(foraging_distance) ) 
mdl %>% group_by(filename,pollinator_species) %>% summarize(foraging_distance = mean(foraging_distance) ) 
