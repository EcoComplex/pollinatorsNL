...
;patch variables
patches-own [
  ;plant-density    ;; in progress
  patch.visited?    ;; true or false statement to record visits to colored patches i.e. plants
  number-of-visits  ;; number of visits recorded
  ;is-flowering     ;; in progress
]
...
to setup
  ...
  if (file-exists? "C:\\Users\\skortsch\\Documents\\Post doc Helsinki Uni\\NetLogo model\\Netlogo test landscapes\\output_folder\\output_mixed landscape.txt") ;delete previous file
  [
    carefully
    [file-delete "C:\\Users\\skortsch\\Documents\\Post doc Helsinki Uni\\NetLogo model\\Netlogo test landscapes\\output_folder\\output_mixed landscape.txt"]
    [print error-message]
  ]


  file-open "C:\\Users\\skortsch\\Documents\\Post doc Helsinki Uni\\NetLogo model\\Netlogo test landscapes\\output_folder\\output_mixed landscape.txt"
  ;file-open "C:\\Users\\skortsch\\Documents\\Post doc Helsinki Uni\\NetLogo model\\Netlogo test landscapes\\output_folder\\output_segregated landscape.txt"
...
to setup-patches
 if landscape = "heterogenous" [
    ...
    set plant-number 160 / 4  ; ( 61 * 61 ) / 9 ; to make a proportion of the patches a plant
 ...
 if landscape = "homogenous" [
    ...
    set plant-number 160 / 4 ;(( 60 * 60 ) / 9) / 4 ; to make a proportion of the patches a plant
...
to setup-pollinators
 ...
 to go
  ...
  move-pollinators
  death
  ;patch-flowering ; will be developed

  tick
end

;phenology prcedure is under construction
;phenology procedure
;to patch-flowering
;  ask patches [
;     (ifelse
...
to move-pollinators
 ...
 ask pollinators with [species = "hoverfly"]  [
   ...
   ask pollinators with [species = "wasp"]  [
    move-wasp
    eat
    count-visits-wasp
...
to count-visits-bee
   ...
   if pcolor != white [if count pollinators-here with [species = "bee"] > 0
      [set patch.visited? TRUE
      set number-of-visits (number-of-visits + 1) ]
      set plabel number-of-visits
      file-type (word species ";")
      file-print pcolor
      ]
end

to count-visits-hfly
   if pcolor != white and pcolor != green [ if count pollinators-here with [species = "hoverfly"] > 0
      [set patch.visited? TRUE
      set number-of-visits (number-of-visits + 1) ]
      set plabel number-of-visits
      file-type (word species ";")
...
to count-visits-bbee
   if pcolor != white and pcolor != yellow [ if count pollinators-here with [species = "bumblebee"] > 0
      [set patch.visited? TRUE
      set number-of-visits number-of-visits + 1 ]
      set plabel number-of-visits
      file-type (word species ";")
...
to count-visits-wasp
if pcolor != white and pcolor != yellow and pcolor != magenta [ if count pollinators-here with [species = "wasp"] > 0
      [set patch.visited? TRUE
      set number-of-visits number-of-visits + 1 ]
      set plabel number-of-visits
      file-type (word species ";")
...
The purpose of our model is to understand how plant and pollinator overlap (or non-overlap), as well as the spatial distribution of plants (mixed vs. segregated), affects visitation rates by pollinators to plants and plant-pollinator network structure. (To begin with, only visitation rates will be modeled, however the model may be extended to account for pollination success by specifying some rules for successful pollination in which a visit alone does not guarantee pollination.)
The model serves a practical tool to mimic climate-induced changes in plant-pollinator phenologies (i.e. overlap/non-overlap between plants and pollinators); pollinator foraging behavior and pollinator interactions with spatially distributed plant species. Pollination visits by different pollinator species to plants can be counted and used to construct plant-pollinator networks that can be analyzed in precisely the same way as conventional field data.
